...
Здесь важно еще раз подчеркнуть, что $ C $ --- просто некая свободная переменная. Удаляя упоминания фокуса из семантического значения, мы также решаем проблему, связанную со случаями употребления $ only $ без эксплицитно выраженного фокуса (подробнее об этом см. раздел \ref{focusPragmatics}).
...
\subsubsection{Обзор прагматических подходов к анализу фокуса}

В разделе \ref{alternativeSemantics} упоминалось, что в рамках семантики альтернатив \citep{rooth1985association,rooth1992theory} фокус не устанавливает значение свободной переменной, а лишь накладывает на него определенные ограничения (например, на тип элементов, которые могут входить во множество альтернатив). 
