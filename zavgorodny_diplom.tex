...
\begin{exe}
    \ex  \gll Gritó e incluso dio una patada al hombre.\footnote{Интересно, что наиболее близким переводом данного предложения на русский язык будет вариант с употреблением \textit{даже}: \textit{и даже ударил его}} \\
...
\begin{exe}
    \ex \begin{xlist}
        \ex \gll Juan estaba muy contento, pero no llegó a ir a la fiesta. \\
...
В первом случае отрицается событие. Это можно проиллюстрировать на следующем примере:
...
\citep{fabergas2017building} замечают, что не все события имеют соответствующее отрицательное событие. Отрицательная интерпретация события возможна только в том случае, если сущность, понимаемая как его субъект, может контролировать инициацию этого события. Это доказывается на примерах трёх типов: (i) невозможность прочтения с отрицательным событием для глаголов, описывающих события без контроллёра; (ii) возможность отрицательного события в случае инхоативно-каузативной альтернациии только в каузативном прочтении; (iii) невозможность отрицательного события при непрямом каузаторе.
