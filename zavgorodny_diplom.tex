...
Очевидно, что в (\ref{strong}) свободная переменная $ C $ чаще всего определяется набором фокусных альтернатив, вводящихся в пресуппозицию на этапе синтаксической деривации (см. (\ref{tildaTree})). Преимуществом данного подхода (помимо эстетических соображений) является то, что в сильной теории мы не вынуждены утверждать, что чувствительные к фокусу выражения могут употребляться только в контекстах с эксплицитным грамматическим фокусом; существование \textit{безакцентного} фокуса и \textit{ассоциации с пресуппозицией} могут служить аргументом в пользу сильных теорий:

\begin{exe}
    \ex \begin{xlist}
        \ex A: Everyone already knew that Mary only eats [\textsc{vegetables}]\textsubscript{F}.
        \ex B: If even [\textsc{Paul}]\textsubscript{F} knew that Mary only eats [\textsc{vegetables}]\textsubscript{SOF}, then he should have suggested a different restaurant.
    \end{xlist}
\end{exe}
